...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-dir", required=True, help="Dossier cible")
    parser.add_argument(
        "--model-path",
        default="./outputs/training/convnext_small_v4/best_model.pt",
        help="Checkpoint du modele a utiliser",
    )
    parser.add_argument("--output-csv", required=True, help="CSV de sortie")
    parser.add_argument("--top-k", type=int, default=3, help="Nombre de classes top-k")
    parser.add_argument(
        "--min-confidence",
        type=float,
        required=True,
        help="Confiance minimale incluse",
    )
    parser.add_argument(
        "--max-confidence",
        type=float,
        required=True,
        help="Confiance maximale exclue",
    )
...
def main() -> None:
    ...
    if args.min_confidence >= args.max_confidence:
        raise ValueError("min-confidence doit etre strictement inferieur a max-confidence")
    ...
    for image_path in iter_images(input_dir):
        ...
        should_move = (
            ...
            and confidence < args.max_confidence
        )
